...
def single_download(quality, referer_url, res):
    ...
    # 获取视频信息
    video_info_temp = JsonUtil.get_video_info(
        res_data.text, '__playinfo__=(.*?)</script><script>')

    video_info = JsonUtil.getJson(video_info_temp, quality)

    VideoUtil.download_video_single(referer_url, video_info.get(
        'video_url', 0), video_info.get('audio_url', 0), title)
...
def batch_download(id, quality):
    res_data = getRequestHtml(
        'https://www.bilibili.com/video/' + id + '?p=%d' % (1))
    res_html = etree.HTML(res_data.text)
    title_list = JsonUtil.get_video_info_title(
        res_data.text, '__INITIAL_STATE__=(.*?),"insertScripts":')
    # 获取当前视频的title
    ...
    for i in range(0, len(video_name_list)):
        # 请求视频，获取信息
        res = requests.get('https://www.bilibili.com/video/' +
                           id + '?p=%d' % (i+1), headers=getHeaders())
        # 解析出视频详情的json
        video_info_temp = JsonUtil.get_video_info(
        ...
        print('当前下载视频清晰度为{}，时长{}分{}秒'.format(
            video_infos.get('quality', 0), video_minute, video_second))
        title = title_list[i]
        # 获取视频信息
        pool.submit(download_video_batch, 'https://www.bilibili.com/video/' + id + '?p=%d' %
                    (i+1), video_infos.get('video_url', 0), video_infos.get('audio_url', 0), title, i+1)
